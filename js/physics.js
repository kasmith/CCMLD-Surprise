...
physics.getMoving = function(world) {
  var movings = world.filter(function(obj) { return !obj.static })
  if (movings.length !== 1) {
    console.log('Watch out! There are multiple moving objects! Only the first will be used!')
  }
  return movings[0];
}

physics.getMovingObjectPosition = function(world) {
  var o = physics.getMoving(world);
  return [o.x, o.y];
}

physics.getPath = function(steps, initialWorld) {
  clearWorld();
  applyWorld(initialWorld);
  var pos = []
  var cworld
  for (var s=0; s<steps; s++) {
    ...
    cworld = churchWorld_from_bodyList(world.GetBodyList())
    pos.push(physics.getMovingObjectPosition(cworld))
  }
  return pos
}

physics.checkOcclusion = function(position, radius, occluder) {
  var minx = occluder[0] + radius,
      miny = occluder[1] + radius,
      maxx = occluder[0] + occluder[2] - radius,
      maxy = occluder[1] + occluder[3] - radius
  return (position[0] >= minx && position[0] <= maxx) && (position[1] >= miny && position[1] <= maxy)
}

physics.observePath = function(steps, initialWorld, radius, occluder) {
  clearWorld();
  applyWorld(initialWorld);
  var output = {
    'location': [],
  ...
  for (var s=0; s<steps; s++) {
    ...
    cworld = churchWorld_from_bodyList(world.GetBodyList())
    pos = physics.getMovingObjectPosition(cworld)
    output['location'].push(pos)
    output['occlusion'].push(this.checkOcclusion(pos, radius, occluder))
  }
  return output
}


physics.animate = function(steps, initialWorld, occluder, fillOccluder) {
  fillOccluder = typeof(fillOccluder !== 'undefined') ? fillOccluder : true
  function simulate(canvas, steps, initializeStep) {
    ...
    function update(stepsSoFar) {
      ...
      if (typeof(occluder) !== 'undefined') {
          ...
          ctx.fillStyle = 'grey'
          ctx.beginPath()
          ctx.rect(occluder[0], occluder[1], occluder[2], occluder[3])
          if (fillOccluder) {
            ctx.fill()
          } else {
            ctx.stroke()
